...
const vector_db_service_1 = __importDefault(require("./vector-db-service"));
class RAGService {
    /**
     * Splits long technical documents into overlapping sentences/chunks for granular search indexes.
     */
    static chunkText(text, chunkSize = 200, chunkOverlap = 50) {
     ...
     */
    static async queryKnowledgeBase(productId, queryText) {
        logger_1.logger.info(`RAG Search - Indexing semantic query: "${queryText}" for product: ${productId}`);
        // Attempt semantic vector query using Embeddings/VectorDB
        try {
            const qVec = embeddings_service_1.default.embedText(queryText);
            // Ensure index loaded
            await vector_db_service_1.default.loadIndex();
            const hits = vector_db_service_1.default.query(qVec, 5);
            const results = [];
            for (const h of hits) {
                // Only return items that belong to productId when possible
                if (h.record.metadata?.productId && String(h.record.metadata.productId) !== productId)
                    continue;
                results.push({
            ...
            logger_1.logger.warn('VectorDB semantic query failed, falling back to textual search');
        }
        // Fallback: classic text-based chunk search
        const kbItems = await kb_model_1.default.find({ productId });
        const results = [];
     ...
     */
    static async performOCR(fileBuffer, fileName) {
        logger_1.logger.info(`OCR Engine - Processing file buffer for: ${fileName}`);
     ...
     */
    static async generateProductPassport(productId) {
        const product = await product_model_1.default.findById(productId).populate('companyId');
        if (!product)
            return null;
        // Calculate a dynamic health score based on category
        let baseHealth = 88;
        if (product.sku.includes('COMP'))
            baseHealth = 74; // Compressor units wear faster
        if (product.sku.includes('BATT'))
            baseHealth = 92;
        const companyName = product.companyId?.name || 'Mantis Manufacturing Corp';
        return {
